...
if __name__ == "__main__":
    # 0) Asegurar que NO hay webhook activo (si lo hay, el polling no recibe nada)
    try:
    ...
    # 1) Levantar Flask en hilo aparte
    Thread(target=run_flask, daemon=True).start()

    # 2) Arrancar polling en el hilo principal (bloqueante)
